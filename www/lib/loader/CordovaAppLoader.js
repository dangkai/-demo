...
/***/ function(module, exports, __webpack_require__) {
	...
	AppLoader.prototype.getAllManifest = function(newManifest){
		var self = this, manifest = this.manifest;
		//console.log("newManifest="+JSON.stringify(newManifest));
		Object.keys(newManifest.files).forEach(function(key){
			manifest.files[key] = newManifest.files[key];
		});
		newManifest.load.forEach(function(src){
			if(manifest.load.indexOf(src) === -1)
				manifest.load.push(src);
		});
		manifest.root = newManifest.root;
		//console.log("new mani fest is : " + JSON.stringify(manifest));
	  return manifest;
	};
	  ...
	  return new Promise(function(resolve,reject){
	    Promise.all([gotNewManifest,self.getBundledManifest(),self.cache.list()])
	      .then(function(values){
			...
			//console.log('oldFiles='+JSON.stringify(oldFiles));
			//console.log('newFiles='+JSON.stringify(newFiles));
			//console.log('bundledFiles='+JSON.stringify(bundledFiles));
	  ...
	  return self.cache.remove(self._toBeDeleted,true)
	    .then(function(){
	      return Promise.all(self._toBeCopied.map(function(file){
	      	console.log(BUNDLE_ROOT + file + "****" + self.cache.localRoot + file);
	...
	AppLoader.prototype.update = function(reload){
	  if(this._updateReady) {
	    // update manifest
		var mani = this.getAllManifest(this.newManifest);
		//alert("update");
	    localStorage.setItem('manifest',JSON.stringify(mani));
